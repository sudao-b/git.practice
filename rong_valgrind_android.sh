...
install_valgrind() {
	...
	echo "DEVICE_CPU_INDEX "$DEVICE_CPU_INDEX
...
if [[ $# -eq 0 ]]; then
	...
	if [[ $# -eq 4 ]]; then
		...
		echo "sss_valgrind_android.sh \"valgrind_parameters\" \"com.example.test\" \"util.LauncherActivity\" \"/lib_with_symbol_table_path\""
		exit 1
	fi
fi
